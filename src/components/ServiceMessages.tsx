...
interface ServiceMessagesProps {
  /** The source name as stored in the DB (e.g. 'discord', 'slack', 'email', 'chatgpt', 'openclaw', 'imessage', 'anthropic') */
  source: string;
  serviceLabel: string;
  serviceIcon: string;
}

export default function ServiceMessages({ source, serviceLabel, serviceIcon }: ServiceMessagesProps) {
  const filterKey = `filters:messages:${source}`;
  const FILTER_DEFAULTS = { q: '', searchInput: '', sender: '', recipient: '', dateFrom: '', dateTo: '' };
  const [filters, setFilters, resetFilters, isDirty] = usePersistedFilters(filterKey, FILTER_DEFAULTS);
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    const params = new URLSearchParams({ page: String(page), limit: String(PAGE_SIZE), sort: 'timestamp', order: 'desc', source });
  ...
  }, [page, filters.q, filters.sender, filters.recipient, filters.dateFrom, filters.dateTo, source]);
  ...
  return (
    <div>
      <h1 className="page-title">{serviceIcon} {serviceLabel} Messages</h1>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table>
          <thead><tr>
            {['ID', 'Sender', 'Recipient', 'Content', 'Timestamp'].map(h =>
              <th key={h}>{h}</th>)}
          </tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={5} style={{ padding: 20, textAlign: 'center', color: '#888' }}>Loading…</td></tr>
              : messages.length === 0 ? <tr><td colSpan={5} style={{ padding: 20, textAlign: 'center', color: '#888' }}>No messages found.</td></tr>
              : messages.map(m => (
