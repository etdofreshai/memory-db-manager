...
export default function SlackLoginStatus() {
  ...
  const handleStop = async () => {
    try {
      await fetch('/proxy/slack-ingestor/api/login/stop', { method: 'POST' });
  ...
  const label = () => {
    ...
    if (status.status === 'logged_in') return status.teamName || status.username || 'Logged in';
  ...
  return (
    <div style={{ padding: '4px 12px 6px', display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, flexWrap: 'wrap' }}>
      ...
      <button
        onClick={() => window.open('/proxy/slack-ingestor/login', '_blank')}
        title="Open Slack login"
