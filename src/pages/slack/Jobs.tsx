...
export default function SlackJobs() {
  ...
  const resolveChannel = useCallback((channelId: string | undefined, chList: ChannelInfo[]) => {
    if (!channelId) return '—';
    const ch = chList.find(c => c.id === channelId);
    return ch ? `#${ch.name}` : channelId;
  }, []);

  const normalizeBackfill = useCallback((r: BackfillRun, chList: ChannelInfo[]): UnifiedJob => {
    const isActive = ['running', 'paused'].includes(r.status?.toLowerCase());
    return {
      id: r.runId,
      type: 'Backfill',
      channelId: r.channelId,
      channelName: resolveChannel(r.channelId, chList),
      status: r.status,
      startedAt: r.startedAt,
      completedAt: r.completedAt,
      duration: durationStr(r.startedAt, r.completedAt),
      messages: r.stats?.totalMessages || r.stats?.downloadedAttachments || 0,
  ...
  const normalizeSyncRun = useCallback((r: SyncRun, chList: ChannelInfo[]): UnifiedJob => {
    const chId = r.channel || r.channelId;
    const chName = r.channelName ? `#${r.channelName}` : resolveChannel(chId, chList);
  ...
  const fetchData = useCallback(async (chList?: ChannelInfo[]) => {
    try {
  ...
  }, [channels, normalizeBackfill, normalizeSyncRun]);

  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const chData = await slackApi<ChannelInfo[]>('/api/channels');
        const cl = Array.isArray(chData) ? chData : [];
        setChannels(cl);
        await fetchData(cl);
      } catch (e: any) {
        setError(e.message);
  ...
  const handleForceCancel = async (job: UnifiedJob) => {
    try {
      await slackApi(`/api/backfill/runs/${job.id}/force-cancel`, { method: 'POST' });
      await fetchData();
    } catch (e: any) {
