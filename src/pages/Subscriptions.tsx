...
export default function Subscriptions() {
  const [activeTab, setActiveTab] = useState<ServiceKey>('discord');
  ...
  const fetchSubscriptions = useCallback(async (service: ServiceKey) => {
    setLoading(true);
    setError('');
    setLocalChanges({});
    try {
  ...
  // Compute effective subscribed state (local override or DB value)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await apiFetch(`/api/subscriptions/${activeTab}`, {
        method: 'PUT',
    ...
    <div>
      ...
      <div style={{
        ...
        {SERVICES.map(svc => (
          <button
            ...
            style={{
              padding: '8px 16px',
              background: activeTab === svc.key ? '#1a2a3a' : 'transparent',
              border: activeTab === svc.key ? '1px solid #4a9eff' : '1px solid #444',
              borderRadius: 6,
              color: activeTab === svc.key ? '#4a9eff' : '#aaa',
              cursor: 'pointer',
              fontSize: 13,
              fontWeight: activeTab === svc.key ? 600 : 400,
      ...
      {loading ? (
        ...
        <div className="card" style={{ textAlign: 'center', padding: 40, color: '#888' }}>
          <p style={{ fontSize: 16, marginBottom: 8 }}>No subscriptions for {SERVICES.find(s => s.key === activeTab)?.label || activeTab}</p>
          <p style={{ fontSize: 13 }}>
